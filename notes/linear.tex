...
\noindent
\begin{sidework}
	\emph{Proof:} Here's a constructive proof from \url{https://math.stackexchange.com/questions/252367/insightful-proofs-for-sherman-morrison-formula-and-matrix-determinant-lemma}. Say you want to solve for $x$
\begin{align}
	(A + uv^T) x & =  y\\
	x & = A^{-1} y - A^{-1} u v^T x
\end{align}
...
\subsubsection{Example: Maximum Likelihood of Gaussian}
...
\begin{align}
	y - X\beta = \epsilon \sim \mathcal N(0, \Sigma) \implies y \sim \mathcal N(X\beta, \Sigma)
\end{align}
Now we can write out $p(y | \beta)$ exactly. Let's write the log form, because argmax is invariant under monotonic functions.
\begin{align}
	\log p(y | \beta) &= - \frac{1}{2}(y - X \beta)^T \Sigma^{-1} (y - X \beta) + \text{Constant w.r.t } \beta
\end{align}
\begin{sidework}
	...
	\begin{align}
		\log p(y |\beta) \propto - \frac{1}{2} (y - \hat y)^2 = \sum_{i} (y_i - \hat y_j)^2
	\end{align}
	I've notated $\hat y = X\beta$. Notice this is your \emph{minimize squared error} from physics 1 lab! So when you implement that for work, you're really trying to construct the maximum likelihood estimation s.t. your data has Gaussian noise.
\end{sidework}
Let's now analytically maximize the log probability.
\begin{align}
	\frac{\partial}{\partial \beta} \log p(y | \beta)& = X^T\Sigma^{-1} (y - X \beta)
\end{align}	
I'm sorry, my notation is consistent with the statisticians notation, where $\partial / \partial \boldsymbol{\beta}$ is a column vector not row vector---not what I showed you earlier.
Since we want to find $\beta = \hat \beta$ when we're maximizing the function, we set the derivative to zero.
\begin{align}
	\beta = \boxed{\hat \beta = (X^T \Sigma^{-1} X)^{-1} X^T \Sigma^{-1} y , \text{ when } n < p}
\end{align}
...
\begin{align}
	\boxed{\hat \beta = \Sigma^{-1} X^T (X \Sigma^{-1} X^T)^{-1} y, \text{ when } p > n}
\end{align}
...
\begin{itemize}
	\item Matrix multiplication: $\mathcal O(n^{2.6})$.
	\item Matrix inverse implemented in \texttt{numpy.linalg.solve}: $\mathcal O(n^{2.6})$.
...
\subsubsection{Condition Number \& Matrix Inversion}
...
\begin{align}
	\text{Cond}(A) = \frac{\max(\lambda_i)}{\min(\lambda_i)}
\end{align}
...
\begin{align}
	A = U \Lambda U^T  = U \, \text{diag}(\lambda_1, ...,\lambda_d) U^T \implies A^{-1} = U \, \text{diag}(1/\lambda_1, ...,1/\lambda_d) \,  U^T
\end{align}
