...
\begin{enumerate}
	...
	\item For high-dimensional distributions the computational budget required of Markov Chain tend to grow as $d$ (in particular $d^{1/4}$ for HMC). ML methods thrive in high dimensional environments, so perhaps there's a nice integration here?
\end{enumerate}
...
Any of these measurements will equal zero i.f.f. the two distributions are equal, and  they're all non-negative, which makes them good loss functions for neural networks. As hinted earlier, the KL Divergence is preferred for computational easyness. The question becomes which argument do the, parameterized distribution $p_\theta$ and target distribution $\pi$, go?
\begin{align}
